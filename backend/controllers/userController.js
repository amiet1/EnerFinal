...
const createUser = async (req, res) => {
  const { first_name, last_name, phone_number, password, email, address } = req.body;
  console.log(req.body);
  const hashPassword = await bcrypt.hash(password, 10)
  try {
    const result = await newUser(first_name, last_name, phone_number, hashPassword, email, address);
...
const updateUserById = async (req, res) => {
  ...
  const { first_name, last_name, phone_number, password, email, address } = req.body;
  try {
    const result = await updateUser(id, first_name, last_name, phone_number, password, email, address);
    res.json(result);
  } catch (Error) {
    res.json({ Error });
  }
